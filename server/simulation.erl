...
start_game({P1Proc, P2Proc} = Info) ->
    ...
    Self = self(),
    server:start_game(P1Proc, {{100, 400, 0}, {400, 400, math:pi()}}),
    server:start_game(P2Proc, {{400, 400, math:pi()}, {100, 400, 0}}),
    game(
        Info,
        [],     %Powerups
        P1State,%P1
...
game(GameInfo, Powerups, P1State, P2State, P1Keys, P2Keys, Points, Timer, Ticker, Golden, Flag) ->
    ...
    receive
        ...
        tick ->
            ...
            server:positions({X1_, Y1_, Alfa1_}, {X2_, Y2_, Alfa2_}, GameInfo, self()),
                ...
                true -> % else check_player_colision
                    ...
                    case {gen_random_box({X1_, Y1_, null}, {X2_, Y2_, null}, Powerups, {Boundx_max, Boundy_max}), HitList1 ++ HitList2} of
                        ...
                        {Powerup, HitList} ->
                            ...
                            server:boxes(Powerup, HitList, GameInfo, self())
                    end,
                    ...
                    case check_player_colision({X1, Y1}, {X2, Y2}, Alfa1, Alfa2) of
                        hit1 ->
                            ...
                            server:positions({X1_, Y1_, Alfa1_}, {X_, Y_, 0}, GameInfo, self()),
                            server:score(P1 + 1, P2, GameInfo, self());
                        ...
                        hit2 ->
                            ...
                            server:positions({X_, Y_, 0}, {X2_, Y2_, Alfa2_}, GameInfo, self()),
                            server:score(P1, P2 + 1, GameInfo, self());
